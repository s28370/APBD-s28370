...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(emp => emp.Salary == Task3());
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps
                ...
                .Select(group => new { name = Depts.FirstOrDefault(d => d.Deptno == group.Key)?.Dname.ToUpper() ?? "UNKNOWN", numOfEmployees = group.Count() });
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.OrderBy(dept => dept.Dname)
                .Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 0 || Emps.Count(emp => emp.Deptno == dept.Deptno) == 5);
            return result;
        }
        ...
        public static IEnumerable<Dept> Task15()
        {
            IEnumerable<Dept> result = Emps
                ...
                .Select(group => new Dept { Dname = group.Key, Loc = group.Count().ToString() });
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Emps
                .SelectMany(emp => Depts, (emp, dept) => dept);
            return result;
        }
    }
